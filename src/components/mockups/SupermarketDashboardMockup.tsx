...
export default function SupermarketDashboardMockup() {
  return (
    <div className="w-full h-full bg-[#050505] flex flex-col font-sans select-none overflow-hidden text-white/90 rounded-3xl border border-white/10 shadow-2xl">
      {/* SaaS Header */}
      <div className="h-14 border-b border-white/10 flex items-center justify-between px-6 bg-white/[0.02] backdrop-blur-md relative z-20">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-white text-sm tracking-tight">NarenNet POS</span>
          </div>
          
          <div className="hidden md:flex items-center gap-1 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-white/40">
            <Search size={14} />
            <span className="text-[11px] px-2 font-medium">Quick Product Search...</span>
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-48 border-r border-white/10 bg-white/[0.01] hidden lg:flex flex-col p-4 gap-1">
          ...
          <div className="mt-auto">
            <div className="p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-12 h-12 bg-emerald-500/10 rounded-full -translate-y-4 translate-x-4 blur-xl" />
              <p className="text-[10px] font-bold text-white mb-2 relative z-10 font-mono tracking-widest uppercase opacity-60">Status</p>
              <div className="flex items-center gap-2 text-emerald-400">
                <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                <span className="text-[11px] font-bold uppercase tracking-wider">Sync Active</span>
              </div>
              <p className="text-[9px] text-white/40 mt-1">42 Bills Offline Sync</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-white/[0.02] flex flex-col gap-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white tracking-tight">Main Branch Dashboard</h1>
              <p className="text-xs text-brand-accent/60 flex items-center gap-2 mt-1 font-medium">
                <Clock size={12} className="animate-pulse" />
                Live Sales Monitoring Active
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard label="Total Revenue" value="₹1.4M" trend="+15.2%" trendUp />
            <MetricCard label="Active Counters" value="4 Counters" trend="High Volume" trendUp />
            <MetricCard label="Low Stock SKUs" value="12 Items" trend="-2.1%" />
            <MetricCard label="Today's Walkins" value="1,284" trend="+8.4%" trendUp />
          </div>
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-[24px] overflow-hidden flex-1 flex flex-col min-h-[300px] shadow-inner">
            <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
              <span className="text-[11px] font-bold text-white/60 uppercase tracking-widest">Live POS Activity</span>
              <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-white/40">
                <span className="text-brand-accent border-b-2 border-brand-accent pb-1 cursor-pointer">Counters</span>
                ...
                <div key={i} className="p-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center font-bold text-brand-accent text-xs group-hover:bg-brand-accent/20 group-hover:border-brand-accent/40 transition-all">
                       {activity.name.charAt(activity.name.length - 1)}
                    </div>
                    <div>
